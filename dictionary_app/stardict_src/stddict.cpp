...
#include "getuint32.h"
#include <QDebug>
#define __DEBUG__ qDebug() << __FILE__ << ":" << __LINE__ << ":" << __func__ << "() "
	...
	for (iLib=0; iLib < dictmask.size(); iLib++) {
		...
		if ( poCurrentWord == NULL ) {
			...
			word = poGetWord(iCurrent[iLib].idx_suggest, iRealLib, servercollatefunc);
                        //__DEBUG__<<"word from poGetWord: "<<word;
		...
		if (poCurrentWord == NULL ) {
			...
			if (x > 0 ) {
				...
				isLib=false;
                        ...
                        //__DEBUG__<<"poCurrentWord: "<<poCurrentWord;
		}
	}
        if (poCurrentWord)
        {
...
bool Libs::LookupSimilarWord(const gchar* sWord, glong & iWordIndex, glong &idx_suggest, size_t iLib, int servercollatefunc)
{
	...
	if (!bFound) {
		...
		if (!bFound) {
			...
			if (!bFound) {
                                if (iIndex!=INVALID_INDEX)
                                {
                                        //__DEBUG__<<"iIndex: "<<iIndex;
                                        cword = poGetWord(iIndex, iLib, servercollatefunc);
                                        //__DEBUG__<<"iIndex: "<<iIndex<<", cword: "<<cword;
                                        if (stardict_casecmp(cword, sWord, EnableCollationLevel, CollateFunction, servercollatefunc)==0)
                                        {
                                            //__DEBUG__<<"iIndex: "<<iIndex<<", cword: "<<cword<<", sWord: "<<sWord;
                                            bFound=true;
					}
				}
			}
	...
	if (bIsPureEnglish(sWord)) {
		...
		if (!bFound && iWordLen>3) {
			...
			if (isupcase || (!strncmp(&sWord[iWordLen-3],"est", 3))) {
                ...
                //__DEBUG__<<"iIndex: "<<iIndex<<", idx_suggest: "<<idx_suggest ;
		g_free(sNewWord);
	}
	...
	if (bFound)
        {
            iWordIndex = iIndex;
//            __DEBUG__<<"bFound: "<<bFound<<", iWordIndex: "<<iWordIndex<<" ,idx_suggest"<<idx_suggest;
